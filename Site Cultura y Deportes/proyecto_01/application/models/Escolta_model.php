 ...
 * Time: 02:06 AM
 */
class Escolta_model extends CI_Model
{
    ...
    public function addAlumno($mat, $file_info, $nom, $dir, $tel, $email, $carr, $niv){
        ...
        return $this->db->insert('escolta', $data);
    }

    //----------------Obtener registro------------//
    public function getAlumno($id = null){
        $this->db->select('*');
        $this->db->from('escolta');
        if ($id != null){
    ...
    public function tuExel(){
        $fields = $this->db->field_data('escolta');
        $this->db->order_by('id_alumno','asc');
        $query =$this->db->get('escolta');
