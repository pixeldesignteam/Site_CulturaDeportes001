 ...
 * Time: 02:06 AM
 */
class Guitarra_model extends CI_Model
{
    ...
    public function addAlumno($mat, $file_info, $nom, $dir, $tel, $email, $carr, $niv){
        ...
        return $this->db->insert('guitarra', $data);
    }

    //----------------Obtener registro------------//
    public function getAlumno($id = null){
        $this->db->select('*');
        $this->db->from('guitarra');
        if ($id != null){
    ...
    public function tuExel(){
        $fields = $this->db->field_data('guitarra');
        $this->db->order_by('id_alumno','asc');
        $query =$this->db->get('guitarra');
