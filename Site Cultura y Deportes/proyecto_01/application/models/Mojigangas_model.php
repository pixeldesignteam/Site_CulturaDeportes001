 ...
 * Time: 02:06 AM
 */
class Mojigangas_model extends CI_Model
{
    ...
    public function addAlumno($mat, $file_info, $nom, $dir, $tel, $email, $carr, $niv){
        ...
        return $this->db->insert('mojigangas', $data);
    }

    //----------------Obtener registro------------//
    public function getAlumno($id = null){
        $this->db->select('*');
        $this->db->from('mojigangas');
        if ($id != null){
    ...
    public function upAlumno($id, $mat ,$nom, $dir, $tel, $email, $carr, $niv){
        ...
        $this->db->where('id_alumno', $id);
        $this->db->update('mojigangas', $data);
    ...
    public function tuXML(){
        $this->load->dbutil();
        $consulta = $this->db->get('mojigangas');
        $config = array (
            'root' => 'mojigangas',
            'element' => 'elemento',
            'newline' => "\n",
    ...
    public function tuExel(){
        $fields = $this->db->field_data('mojigangas');
        $this->db->order_by('id_alumno','asc');
        $query =$this->db->get('mojigangas');
