<?php

defined('BASEPATH') OR exit('No direct script access allowed');

class Coro_model extends CI_Model{

    public function __construct() {
        parent::__construct();
    }


    public function getCoro($id = null){
      $this->db->order_by('nombre_alumno', 'asc');
      if ($id != null ) {
        $this->db->where('id_alumno', $id);
      }
      $coro = $this->db->get('coro');

      if ($coro->num_rows() > 0):
        return $coro->result();
      endif;
    }

    public function addCoro($m,$file_info,$n,$d,$t,$e,$c,$ni){
    $data = array(
        'id_alumno' => 0,
        'matricula'  => $m,
        'foto'  => $file_info,
        'nombre_alumno'  => $n,
        'direccion'  => $d,
        'telefono'  => $t,
        'email'  => $e,
        'carrera'  => $c,
        'nivel'  => $ni
    );
    return $this->db->insert('coro', $data);

}

public function upCoro($id,$m,$n,$d,$t,$e,$c,$ni){

    $data = array(
      'matricula'  => $m,
      'nombre_alumno'  => $n,
      'direccion'  => $d,
      'telefono'  => $t,
      'email'  => $e,
      'carrera'  => $c,
      'nivel'  => $ni

    );
    $this->db->where('id_alumno', $id);
    return $this->db->update('coro', $data);
}


public function delCoro($id){
    $this->db->where('id_alumno', $id);

    return $this->db->delete('coro');
}

public function tuXML(){
    $this->load->dbutil();
    $consulta = $this->db->get('coro');
    $config = array(
        'root' => 'proyecto_01',
        'element' => 'elemento',
        'newline' => "\n",
        'tab' => "\t",

    );

    $respuestaXML = $this->dbutil->xml_from_result($consulta, $config);
    return $respuestaXML;
}





  }
