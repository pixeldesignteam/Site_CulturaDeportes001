...
class Mojigangas extends CI_Controller{
	public function __construct() {
        parent::__construct();
        $this->load->model('Admin_model/Moji_model');
    }

    public function getAlumno($id=null){
			if($this->session->userdata('autentificado')){
      $data['alumno'] = $this->Moji_model->getAlumno();
      $this->load->view('admin/mojigangas', $data);
		}else {
  ...
  public function nuevoAlumno(){
    if($this->session->userdata('autentificado')){
     $this->load->view('admin/mojigangas/nuevoAlumno');
 }  else {
     redirect('Home/index');
 }
  ...
  public function addAlumno(){
      ...
      $c = $this->input->post('carrera');
			...
			if(!$this->upload->do_upload('foto')){
		 		...
		 		$this->Moji_model->addAlumno($m,$file_info['file_name'],$n,$d,$t,$e,$c,$ni);

		 		redirect('CA/Mojigangas/getAlumno');
		 }
  ...
  public function actualizarAlumno($id){
		if($this->session->userdata('autentificado')){
      $data['alumno'] = $this->Moji_model->getAlumno($id);
      $this->load->view('admin/mojigangas/actualizarAlumno', $data);
  }else {
...
public function upAlumno(){
    ...
    $c = $this->input->post('carrera');
		...
		$this->Moji_model->upAlumno($id,$m,$n,$d,$t,$e,$c,$ni);

			redirect('CA/Mojigangas/getAlumno');}

  public function delAlumno($id){
      $this->Moji_model->delAlumno($id);
			redirect('CA/Mojigangas/getAlumno');
  }

	public function tuXML($nombre){
			$xml = $this->Moji_model->tuXML();
