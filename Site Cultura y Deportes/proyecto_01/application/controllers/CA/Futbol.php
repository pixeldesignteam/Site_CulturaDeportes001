...
class Futbol extends CI_Controller{
	public function __construct() {
        parent::__construct();
        $this->load->model('Admin_model/Futbol_model');
    }

    public function getAlumno($id=null){
			if($this->session->userdata('autentificado')){
      $data['alumno'] = $this->Futbol_model->getAlumno();
      $this->load->view('admin/futbol', $data);
		}else {
  ...
  public function nuevoAlumno(){
    if($this->session->userdata('autentificado')){
     $this->load->view('admin/futbol/nuevoAlumno');
 }  else {
     redirect('Home/index');
 }
  ...
  public function addAlumno(){
      ...
      $c = $this->input->post('carrera');
			...
			if(!$this->upload->do_upload('foto')){
		 		...
		 		$this->Futbol_model->addAlumno($m,$file_info['file_name'],$n,$d,$t,$e,$c,$ni);

		 		redirect('CA/Futbol/getAlumno');
		 }
  ...
  public function actualizarAlumno($id){
		if($this->session->userdata('autentificado')){
      $data['alumno'] = $this->Futbol_model->getAlumno($id);
      $this->load->view('admin/futbol/actualizarAlumno', $data);
  }else {
...
public function upAlumno(){
    ...
    $c = $this->input->post('carrera');
		...
		$this->Futbol_model->upAlumno($id,$m,$n,$d,$t,$e,$c,$ni);
