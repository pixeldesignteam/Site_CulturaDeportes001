...
class Escolta extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('Escolta_model');
    }

    //-------------Cargar Vista Login-------------//
    public function login_taller(){
        if($this->session->userdata('autentificado')) {
            $data['content'] = 'talleres/login_Taller/escolta';
    ...
    //-------------Cargar Vista Escolta------------//
    public function escolta ($id = null){
        if($this->session->userdata('autentificado')) {
            $data['alum'] = $this->Escolta_model->getAlumno($id);
            $data['content'] = 'talleres/escolta/inicio';
            $data['nombre'] = $this->session->userdata('nombre');
            $this->load->view('talleres/escolta/plantilla', $data);
        }else{
            redirect('Home/index');
        }
    }
    ...
    public function login_data(){
        if($this->session->userdata('autentificado')) {
            ...
            $profesor = $this->Escolta_model->sesion($matricula, $password);
            ...
            if ($profesor){
                ...
                redirect('CPProfesor/Escolta/escolta');
    ...
    public function addAlumno(){
        if($this->session->userdata('autentificado')) {
            ...
            if (!$this->upload->do_upload('foto')){
                ...
                $this->Escolta_model->addAlumno($mat, $file_info['file_name'] , $nom, $dir, $tel, $email, $carr, $niv);
                redirect('CPProfesor/Escolta/escolta');
                $this->escolta();
            }
        }else{
    ...
    public function frmUpAlumno($id = null){
        if($this->session->userdata('autentificado')) {
            $data['al'] = $this->Escolta_model->getAlumno($id);
    ...
    public function upAlumno(){
        if($this->session->userdata('autentificado')) {
            ...
            $this->Escolta_model->upAlumno($id, $mat ,$nom, $dir, $tel, $email, $carr, $niv);
            redirect('CPProfesor/Escolta/escolta');
        }else{
            redirect('Home/index');
        }
    ...
    public function tuExel(){
        if($this->session->userdata('autentificado')) {
            ...
            to_excel($this->Escolta_model->tuExel(),"ReporteAlumno");
        }else{
            redirect('Home/index');
        }
    }
