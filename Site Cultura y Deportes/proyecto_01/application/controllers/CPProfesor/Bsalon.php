 ...
 * Time: 02:03 AM
 */
class Bsalon extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('Bsalon_model');
    }

    //-------------Cargar Vista Login-------------//
    public function login_taller(){
        if($this->session->userdata('autentificado')) {
            $data['content'] = 'talleres/login_Taller/Bsalon';
    ...
    //-------------Cargar Vista Bsalon------------//
    public function bsalon($id = null){
        if($this->session->userdata('autentificado')) {
            $data['alum'] = $this->Bsalon_model->getAlumno($id);
            $data['content'] = 'talleres/baile_salon/inicio';
            $data['nombre'] = $this->session->userdata('nombre');
            $this->load->view('talleres/baile_salon/plantilla', $data);
        }else{
            redirect('Home/index');
        }
    }
    ...
    public function login_data(){
        if($this->session->userdata('autentificado')) {
            ...
            $profesor = $this->Bsalon_model->sesion($matricula, $password);
            ...
            if ($profesor){
                ...
                redirect('CPProfesor/Bsalon/bsalon');
    ...
    public function addAlumno(){
        if($this->session->userdata('autentificado')) {
            ...
            if (!$this->upload->do_upload('foto')){
                ...
                $this->Bsalon_model->addAlumno($mat, $file_info['file_name'] , $nom, $dir, $tel, $email, $carr, $niv);
                redirect('CPProfesor/Bsalon/bsalon');
                $this->bsalon();
            }
        }else{
    ...
    public function frmUpAlumno($id = null){
        if($this->session->userdata('autentificado')) {
            $data['al'] = $this->Bsalon_model->getAlumno($id);
    ...
    public function upAlumno(){
        if($this->session->userdata('autentificado')) {
            ...
            $this->Bsalon_model->upAlumno($id, $mat ,$nom, $dir, $tel, $email, $carr, $niv);
            redirect('CPProfesor/Bsalon/bsalon');
        }else{
            redirect('Home/index');
        }
    ...
    public function delAlumno($id){
        if($this->session->userdata('autentificado')) {
            $this->Bsalon_model->delAlumno($id);
    ...
    public function tuExel(){
        if($this->session->userdata('autentificado')) {
            ...
            to_excel($this->Bsalon_model->tuExel(),"ReporteAlumno");
        }else{
            redirect('Home/index');
        }
    }
