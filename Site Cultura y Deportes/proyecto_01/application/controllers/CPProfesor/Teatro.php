...
class Teatro extends CI_Controller
{
    ...
    public function login_taller(){
        if($this->session->userdata('autentificado')) {
            $data['content'] = 'talleres/login_Taller/teatro';
    ...
    //-------------Cargar Vista Teatro------------//
    public function teatro($id = null){
        if($this->session->userdata('autentificado')) {
            $data['alum'] = $this->Teatro_model->getAlumno($id);
            $data['content'] = 'talleres/teatro/inicio';
            $data['nombre'] = $this->session->userdata('nombre');
            $this->load->view('talleres/teatro/plantilla', $data);
        }else{
            redirect('Home/index');
        }
    }
    ...
    public function login_data(){
        if($this->session->userdata('autentificado')) {
            ...
            $profesor = $this->Teatro_model->sesion($matricula, $password);
            ...
            if ($profesor){
                ...
                redirect('CPProfesor/Teatro/teatro');
    ...
    public function addAlumno(){
        if($this->session->userdata('autentificado')) {
            ...
            if (!$this->upload->do_upload('foto')){
                ...
                $this->Teatro_model->addAlumno($mat, $file_info['file_name'] , $nom, $dir, $tel, $email, $carr, $niv);
                redirect('CPProfesor/Teatro/teatro');
                $this->teatro();
            }
        }else{
    ...
    public function frmUpAlumno($id = null){
        if($this->session->userdata('autentificado')) {
            $data['al'] = $this->Teatro_model->getAlumno($id);
    ...
    public function upAlumno(){
        if($this->session->userdata('autentificado')) {
            ...
            $this->Teatro_model->upAlumno($id, $mat ,$nom, $dir, $tel, $email, $carr, $niv);
            redirect('CPProfesor/Teatro/teatro');
        }else{
            redirect('Home/index');
        }
    ...
    public function tuExel(){
        if($this->session->userdata('autentificado')) {
            ...
            to_excel($this->Teatro_model->tuExel(),"ReporteAlumno");
        }else{
            redirect('Home/index');
        }
    }
