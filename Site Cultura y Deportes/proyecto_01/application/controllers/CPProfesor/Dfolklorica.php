 ...
 * Time: 02:03 AM
 */
class Dfolklorica extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('Dfolklorica_model');
    }

    //-------------Cargar Vista Login-------------//
    public function login_taller(){
        if($this->session->userdata('autentificado')) {
            $data['content'] = 'talleres/login_Taller/Dfolklorica';
    ...
    //-------------Cargar Vista Dfolklorica------------//
    public function d_folklorica($id = null){
        if($this->session->userdata('autentificado')) {
            $data['alum'] = $this->Dfolklorica_model->getAlumno($id);
            $data['content'] = 'talleres/d_folklorica/inicio';
            $data['nombre'] = $this->session->userdata('nombre');
            $this->load->view('talleres/d_folklorica/plantilla', $data);
        }else{
            redirect('Home/index');
        }
    }
    ...
    public function login_data(){
        if($this->session->userdata('autentificado')) {
            ...
            $profesor = $this->Dfolklorica_model->sesion($matricula, $password);
            ...
            if ($profesor){
                ...
                redirect('CPProfesor/Dfolklorica/d_folklorica');
    ...
    public function addAlumno(){
        if($this->session->userdata('autentificado')) {
            ...
            if (!$this->upload->do_upload('foto')){
                ...
                $this->Dfolklorica_model->addAlumno($mat, $file_info['file_name'] , $nom, $dir, $tel, $email, $carr, $niv);
                redirect('CPProfesor/Dfolklorica/d_folklorica');
                $this->d_folklorica();
            }
        }else{
    ...
    public function frmUpAlumno($id = null){
        if($this->session->userdata('autentificado')) {
            $data['al'] = $this->Dfolklorica_model->getAlumno($id);
    ...
    public function upAlumno(){
        if($this->session->userdata('autentificado')) {
            ...
            $this->Dfolklorica_model->upAlumno($id, $mat ,$nom, $dir, $tel, $email, $carr, $niv);
            redirect('CPProfesor/Dfolklorica/d_folklorica');
        }else{
            redirect('Home/index');
        }
    ...
    public function delAlumno($id){
        if($this->session->userdata('autentificado')) {
            $this->Dfolklorica_model->delAlumno($id);
    ...
    public function tuXML($nombre){
        if($this->session->userdata('autentificado')) {
            $xml = $this->Dfolklorica_model->tuXML();
    ...
    public function tuExel(){
        if($this->session->userdata('autentificado')) {
            ...
            to_excel($this->Dfolklorica_model->tuExel(),"ReporteAlumno");
        }else{
            redirect('Home/index');
        }
    }
