...
class Baloncesto extends CI_Controller
{
    ...
    public function login_taller(){
        if($this->session->userdata('autentificado')) {
            $data['content'] = 'talleres/login_Taller/baloncesto';
    ...
    //-------------Cargar Vista Baloncesto------------//
    public function baloncesto($id = null){
        if($this->session->userdata('autentificado')) {
            $data['alum'] = $this->Baloncesto_model->getAlumno($id);
            $data['content'] = 'talleres/baloncesto/inicio';
            $data['nombre'] = $this->session->userdata('nombre');
            $this->load->view('talleres/baloncesto/plantilla', $data);
        }else{
            redirect('Home/index');
        }
    }
    ...
    public function login_data(){
        if($this->session->userdata('autentificado')) {
            ...
            $profesor = $this->Baloncesto_model->sesion($matricula, $password);
            ...
            if ($profesor){
                ...
                redirect('CPProfesor/Baloncesto/baloncesto');
    ...
    public function addAlumno(){
        if($this->session->userdata('autentificado')) {
            ...
            if (!$this->upload->do_upload('foto')){
                ...
                $this->Baloncesto_model->addAlumno($mat, $file_info['file_name'] , $nom, $dir, $tel, $email, $carr, $niv);
                redirect('CPProfesor/Baloncesto/baloncesto');
                $this->baloncesto();
            }
        }else{
    ...
    public function upAlumno(){
        if($this->session->userdata('autentificado')) {
            ...
            $this->Baloncesto_model->upAlumno($id, $mat ,$nom, $dir, $tel, $email, $carr, $niv);
            redirect('CPProfesor/Baloncesto/baloncesto');
        }else{
            redirect('Home/index');
        }
    ...
    public function delAlumno($id){
        if($this->session->userdata('autentificado')) {
            $this->Baloncesto_model->delAlumno($id);
    ...
    public function tuExel(){
        if($this->session->userdata('autentificado')) {
            ...
            to_excel($this->Baloncesto_model->tuExel(),"ReporteAlumno");
        }else{
            redirect('Home/index');
        }
    }
